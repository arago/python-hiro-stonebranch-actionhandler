...
class TaskInstance:
    status_desc = {
        'CANCEL_PENDING':
            'A process running on the Agent needs to be terminated. When the Cancel command is issued,'
        ...
        'EXECUTION_WAIT':
            'The task must wait to be completed; either the Agent/Agent Cluster running the task'
            ' has reached its Task Execution Limit, or the ability of the Agent/Agent Cluster'
            ' to run tasks has been suspended.',
        'IN_DOUBT':
            'The Agent is "in doubt" about the current status of the task instance. This may occur if an Agent'
        ...
        'EXCLUSIVE_REQUESTED':
            'All tasks with a mutually exclusive task defined go immediately to a status of'
        ...
        'EXCLUSIVE_WAIT':
            'The task is mutually exclusive with one or more other tasks, and it is waiting for those'
            ' tasks to finish before it will run.',
        'FINISHED':
            'The task was forced by the user to finish. The user may do this in cases where the task had a'
        ...
        'RESOURCE_REQUESTED':
            'All tasks with a virtual resource defined go immediately to a status'
        ...
        'SUCCESS':
            'The task has completed successfully. Workflows will transition to Success status when all of its'
        ...
        'ACTION_REQUIRED':
            'When a manual task launches, it goes into Action Required status, meaning a user'
        ...
        'CONFIRMATION_REQUIRED':
            'If you make JCL changes and restart a z/OS task, Universal Controller will put'
        ...
        'SUBMITTED':
            'The task has been submitted to the z/OS Job Entry subsystem and scheduled'
